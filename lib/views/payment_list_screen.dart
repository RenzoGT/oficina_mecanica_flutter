    ...
    return Scaffold(
      ...
      body: Builder(builder: (context) {
        ...
        if (paymentViewModel.errorMessage != null) {
          return Center(child: Text('Error: ${paymentViewModel.errorMessage}'));
        }

        if (invoiceViewModel.errorMessage != null) {
          return Center(child: Text('Error: ${invoiceViewModel.errorMessage}'));
        }
      ...
      floatingActionButton: Builder(
        builder: (context) {
          final paidInvoiceIds = paymentViewModel.payments.map((p) => p.invoiceId).toSet();
          final hasAvailableInvoices = invoiceViewModel.invoices.any((invoice) => !paidInvoiceIds.contains(invoice.id));

          return FloatingActionButton(
            onPressed: hasAvailableInvoices ? () {
              Navigator.push(
                context,
                MaterialPageRoute(
